...
must be in one of the first fifty places they'll look, so in order to save Christmas, you need to help
them get fifty stars on their list before Santa takes off on December 25th.
...
list of locations to check is currently empty. Eventually, someone decides that the best place to check
...
unique number called the location ID. To make sure they don't miss anything, The Historians split into
two groups, each searching the office and trying to create their own complete list of location IDs.
...
For example, if you pair up a 3 from the left list with a 7 from the right list, the distance apart is
...
right list. Calculate a total similarity score by adding up each number in the left list after multiplying
...
So, for these example lists, the similarity score at the end of this process is 31 (9 + 4 + 0 + 0 + 9
+ 9).
...
class _IdLists:
    def __init__(self, lists: tuple[tuple[int, int], ...]) -> None:
        self._left_list = sorted([i[0] for i in lists])
...
class Puzzle(BasicPuzzle):
    ...
    def _test_puzzle(self) -> None:
        ...
        id_list = _IdLists(test_input)
        self._print_test(Fd(11, id_list.calculate_distance, ()))
