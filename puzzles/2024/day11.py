...
As you observe them for a while, you find that the stones have a consistent behavior. Every time you
...
No matter how the stones change, their order is preserved, and they stay on their perfectly straight
line.

How will the stones evolve if you keep blinking at them? You take a note of the number engraved on each
stone in the line (your puzzle input).
...
So, after blinking once, your five stones would become an arrangement of seven stones engraved with the
numbers 1 2024 1 0 9 9 2021976.
...
class _Stones:
    ...
    def _blink_once(self) -> None:
        i = 0
        while i < len(self._stones):
            if self._stones[i] == 0:
                self._stones[i] = 1
                i += 1
                continue

            number = str(self._stones[i])
            length = len(number)
            if length % 2 == 0:
                left = number[:length//2]
                right = number[length//2:]
                self._stones[i] = int(right)
                self._stones.insert(i, int(left))
                i += 2
            else:
                self._stones[i] *= 2024
                i += 1
...
class Puzzle(BasicPuzzle):
    ...
    def _test_puzzle(self) -> None:
        s = _Stones([125, 17])
        self._print_test(Fd(22, s.blink, (6,)))
        self._print_test(Fd(55312, s.blink, (19,)))

    def _solve_puzzle(self) -> None:
        puzzle_input = self.read_file(_compile_data)
        s = _Stones(puzzle_input)
        self._print_result(Fd(217443, s.blink, (25,)))
        self._print_result(Fd(217443, s.blink, (50,)))  # too slow
