...
updates won't print correctly. Failure to update the safety manuals would be dire indeed, so you offer
your services.

Safety protocols clearly indicate that new pages for the safety manuals must be printed in a very specific
order. The notation X|Y means that if both page number X and page number Y are to be produced as part
of an update, page number X must be printed at some point before page number Y.

The Elf has for you both the page ordering rules and the pages to produce in each update (your puzzle
input), but can't figure out whether each update has the pages in the right order.
...
To get the printers going as soon as possible, start by identifying which updates are already in the
right order.
...
Because the first update does not include some page numbers, the ordering rules involving those missing
...
they also do not include every page number, and so only some of the ordering rules apply - within each
...
For some reason, the Elves also need to know the middle page number of each update being printed. Because
you are currently only printing the correctly-ordered updates, you will need to find the middle page
number of each correctly-ordered update. In the above example, the correctly-ordered updates are:
    75,47,61,53,29
    97,61,53,29,13
    75,29,13

These have middle page numbers of 61, 53, and 29 respectively. Adding these page numbers together gives
143.
...
For each of the incorrectly-ordered updates, use the page ordering rules to put the page numbers in the
right order. For the above example, here are the three incorrectly-ordered updates and their correct
orderings:
...
class _Updates:
    def __init__(self, data: str) -> None:
        ...
        self._order = defaultdict(set)
        for line in order_list.splitlines():
            x, y = line.split("|")
            self._order[int(x)].add(int(y))
    ...
    def _has_right_order(self, updates: list[int]) -> bool:
        for index, update in enumerate(updates):
            if self._order[update].intersection(updates[:index]):
                return False
        return True

    def _sort(self, updates: list[int]) -> None:
        for index, update in enumerate(updates):
            if self._order[update].intersection(updates[:index]):
                for i in range(index, 0, -1):
                    updates[i], updates[i - 1] = updates[i - 1], updates[i]
                    if self._has_right_order(updates[:index]):
                        break
...
97,13,75,29,47"""
    ...
    def _solve_puzzle(self) -> None:
        ...
        self._print_result(Fd(5180, updates.sum_incorrect_middle, ()))
