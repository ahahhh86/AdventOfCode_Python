...
struggling with his computer. He's trying to make more contiguous free space by compacting all of the
...
three-block file with ID 1, and a five-block file with ID 2. Using one character for each block where
...
space block (until there are no gaps remaining between file blocks). For the disk map 12345, the process
looks like this:
    0..111....22222
    02.111....2222.
    022111....222..
...
Continuing the first example, the first few blocks' position multiplied by its file ID number are 0 *
0 = 0, 1 * 0 = 0, 2 * 9 = 18, 3 * 9 = 27, 4 * 8 = 32, and so on. In this example, the checksum is the
sum of these, 1928.
...
free space, just like the amphipod hoped. Second, the computer is running much more slowly! Maybe introducing
...
Start over, now compacting the amphipod's hard drive using this new method instead. What is the resulting
filesystem checksum?
...
class _DataMap:
    ...
    def _move_blocks_part2(self) -> list[str | int]:
        # TODO: find problem with code below, would be a lot faster
        # different result:
        # 5402540254025402540254025402548954895489548954895489548954895489
        # correct would be, missing the empty space:
        # 5402540254025402540254025402................................................................548954895489548954895489548954895489
        ...
        first_empty = blocks.index(self._EMPTY_SPACE)
        i = blocks_length - 1
        while i > first_empty:
            if blocks[i] == self._EMPTY_SPACE:
                i -= 1
            ...
            length = 1
            while blocks[i - 1] == blocks[i]:
                i -= 1
                length += 1

            find = [self._EMPTY_SPACE] * length

            for j in range(first_empty, i):
                if blocks[j:j + length] == find:
...
class Puzzle(BasicPuzzle):
    ...
    def _solve_puzzle(self) -> None:
        ...
        self._print_result(Fd(6379677752410, m.calculate_checksum_part2, ()))  # 46880 ms
