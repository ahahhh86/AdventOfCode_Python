...
Your puzzle answer was 21841249.
"""
...
def _jump(instructions: tuple[int, ...], decrease_width: int = None) -> int:
    ...
    jumper = _JumpIndex()
    while jumper.index < length:
        width = instructions[jumper.index]
        if width >= decrease_width:
            instructions[jumper.index] -= 1
        else:
            instructions[jumper.index] += 1
        jumper.jump(width)
    return jumper.steps
